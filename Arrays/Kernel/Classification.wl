...
ArrayLazyQ::usage = "ArrayLazyQ[a] gives True if a is a lazy parametric array container: an inert array-valued expression whose head is registered in the lazy tier. The registered heads are an array-valued InterpolatingFunction (the bare object when it interpolates over a single input coordinate, whose elements are its unapplied component interpolants - a scalar-valued or a multivariate bare object is not a container - and any applied form with at least one non-numeric argument), a fully applied array-valued ParametricFunction pf[params][t] with at least one non-numeric argument, an unapplied array-valued Function whose shape an ArrayDeclareShape declaration, a formal-symbol probe or a numeric probe settles, an array-valued Piecewise whose branch values and default are arrays of one shape, and a source NetGraph or NetChain - one with no open input ports, whose output port reports a positive integer dimension list and whose value is net[]. A structural tree whose leaves are all explicit containers, the deferred form a tensor-network contraction returns unactivated, is lazy as well: it has no registered head, but it has a value and merely defers computing it. Exact numeric arguments such as Pi or 1/2 evaluate an applied form, so only non-numeric arguments stay lazy. Recognizing an unapplied Function EVALUATES it: with no declared shape its body is run at formal symbols and, failing that, at a random point in {0, 1}, so a Function with side effects or an expensive body should be given a shape with ArrayDeclareShape, which is consulted first and skips both probes."

ArraySymbolicQ::usage = "ArraySymbolicQ[a] gives True if a is a symbolic array container: VectorSymbol, MatrixSymbol, ArraySymbol, an atomic symbol registered in $Assumptions as an element of Vectors, Matrices or Arrays, or a structural tree whose nodes are Inactive[D], Transpose, Inactive[TensorProduct], Plus, TensorContract, ArrayContract, Dot, ArrayDot or ArrayReshape and which carries a symbolic container at any depth. A tree over explicit containers alone, the deferred form a tensor-network contraction returns unactivated, has a value and merely defers computing it, so it is a lazy container rather than a symbolic one. Recognizing a structural tree never probes the lazy tier, so it cannot evaluate a Function leaf."
...
(* A structural tree is SYMBOLIC when it carries a symbolic container, and only
   ...
   computed, so it is deferred and belongs to the lazy tier (see ArrayLazyQ).
   Were it to answer True here as well, a tensor-network contraction and a tree
   over a VectorSymbol would be indistinguishable - same tier, same predicates,
