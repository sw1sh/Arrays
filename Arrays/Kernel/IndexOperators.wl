...
(* The executors differ in the effects they admit and in how they read their
   operand argument, and in nothing else: one analysis, one plan, one
   application of the plan's Function to the operands. *)
...
(* The Function this hands back is the one indexExecutePlan applies to the
   operands, so a trace read here is what runs. *)
