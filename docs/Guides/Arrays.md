...
The Arrays paclet treats the many array representations of the Wolfram Language as one family of containers under a single admission criterion: the shape must be introspectable without materializing the elements, and a materialization path must exist. Containers come in three tiers. The explicit tier holds its elements in memory: [SparseArray](), packed and plain [List]() arrays, [NumericArray](), structured arrays such as [SymmetrizedArray](), and shape-introspectable wrapper containers ([QuantityArray](), [TabularColumn](), [Tabular](), [Dataset](), [ByteArray](), [EventSeries]() and [DataStructure]() array stores). The lazy tier is an array-valued expression awaiting its parameters, an inert application or an unapplied form, whose head is registered in the tier: an array-valued [InterpolatingFunction](), applied to a symbolic time or bare over a single input coordinate, a fully applied [ParametricFunction](), an unapplied array-valued [Function](), an array-valued [Piecewise](), and a source [NetGraph]() or [NetChain](); the deferred contraction tree a tensor-network contraction returns unactivated, a structural tree over explicit containers, is lazy as well, having a value it merely defers computing. The symbolic tier carries a symbol and has no values to produce: [VectorSymbol](), [MatrixSymbol](), [ArraySymbol](), symbols registered in [$Assumptions](), and structural trees that carry one of them at any depth. Whether a container also computes natively without materializing is a separate per-head capability flag, not an admission gate. The tiers and the element domains are ordered lattices, so a set of containers of different kinds has a common representation: [ArrayTier]() and [ArrayElementDomain]() read the two coordinates, [ArrayUnify]() joins them over an operand set, and [ArrayCoerce]() moves a single container up either lattice. The structural operations are also reachable by axis name: an index descriptor written in the classic einsum dialect, the einx identifier dialect or the Wolfram list-of-shapes dialect compiles to an [ArrayIndexPattern](), solves its axis sizes against shapes alone as an [ArrayIndexPlan](), and runs over explicit containers as a contraction with [ArrayIndexContract]() or a rearrangement with [ArrayIndexTransform]().
...
- `ArrayLazyQ` test for a lazy container: an array-valued interpolating, parametric, function, piecewise or net expression awaiting its parameters, or a deferred structural tree over explicit containers
- `ArraySymbolicQ` test for a symbolic container: array symbols, assumption-registered symbols, and structural trees carrying them
...
- `ArrayIndexPattern` parse an index descriptor into a normalized pattern object, in the einsum, einx or list-of-shapes dialect
- `ArrayIndexPlan` solve the axis sizes of a descriptor against dimensions or arrays and give an execution plan
- `ArrayIndexContract` contract a set of containers by an index descriptor, summing every axis the output drops
- `ArrayIndexTransform` rearrange one container by an index descriptor, splitting, merging, permuting and repeating axes
